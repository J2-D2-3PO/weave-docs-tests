...
uiiwuuiwuwcniuwnuiecwe

In this tutorial you will learn how to ISATYPO
...
Building on our [basic tracing example](/quicksta), we will now add additional logic to count the returned items from our LLM and wrap them all in a higher level function. We'll then add `weave.op()` to trace every function, its call order and its parent-child relationship:
